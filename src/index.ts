...
interface TransactionsResponse {
    ...
    items: Record<string, BucketModel>;
...
export default class DasBudget {
    ...
    public readonly baseUrl = 'https://api.dasbudget.com';
    ...
    public async transactions(
        ...
        const allTransactions: TransactionModel[] = [];
        let currentPage = 1;
                ...
                if (since !== undefined) {
                    ...
                    allTransactions.push(
                        ...filteredTransactions.map(
                            (tx: Transaction) => new TransactionModel(tx, this)
                        )
                    );
    ...
    private async getBucketsByKind(
        ...
        try {
            ...
            return response.data.items.map(
                (item: Bucket) => new BucketModel(item, this)
            );
        } catch (error) {
    ...
    public async expenses(): Promise<BucketModel[]> {
        return this.getBucketsByKind('expense');
    }

    public async goals(): Promise<BucketModel[]> {
        return this.getBucketsByKind('goal');
    }

    public async vaults(): Promise<BucketModel[]> {
    ...
    public async assignTransactionToBucket(
        ...
        await this.ensureValidToken();
        this.log(
            `Assigning transaction ${transactionId} to bucket ${bucketId}...`
        );

        try {
            const actualBucketId =
                bucketId === FREE_TO_SPEND ? 'fts' : bucketId;
            ...
            return new TransactionModel(response.data, this);
        } catch (error) {
    ...
    public async getAccounts(): Promise<AccountModel[]> {
        ...
        try {
            ...
            return response.data.items.map(
                (item: AccountData) => new AccountModel(item, this)
            );
        } catch (error) {
